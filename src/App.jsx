...
const ArwIco=()=><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="7" y1="17" x2="17" y2="7"/><polyline points="7 7 17 7 17 17"/></svg>

/* ── Data ────────────────────────────────────────────────── */
const EXP=[
  {company:'Feather Engine',tag:'Open Source',role:'Maintainer',date:'2025 — Present',desc:'Bash-based ColorOS/OxygenOS 16 porting framework for SM8350/SM8250. Smali AI patching, OTA generation, SuperVOOC 65W paths, premium feature-gating. Named after Sabrina Carpenter\'s "Feather."',tags:['Bash','SM8350','ColorOS 16','OTA','Smali']},
  {company:'ReVork',tag:'Community',role:'Founder',date:'2025 — Present',desc:'~260-member Telegram community for custom ROM development. Sabrina Carpenter-themed role system, ROM release channels, developer support.',tags:['Community','Android','Telegram']},
  ...
  {company:'patidar.ozyern.me',tag:'Web Dev',role:'Developer',date:'Ongoing',desc:'Dark luxury red/gold fan site for RCB\'s Rajat Patidar. Dynamic Island nav, detailed animations. Cold outreach to management planned.',tags:['HTML/CSS/JS','RCB']},
]
const PROJ=[
...
const SKILLS=[
  {icon:'🔧',name:'Android ROM Dev',num:'01',desc:'Low-level porting, partition manipulation, smali patching, OTA generation. Primary: SM8350 / OnePlus 9 Pro.',tags:['port.sh','Smali','ADB','OTA','ColorOS 16']},
  {icon:'⚙️',name:'Kernel & Modules',num:'02',desc:'Feather Kernel, binary string patching, SukiSU Ultra spoof modules, KernelSU/Magisk module framework, AnyKernel packaging.',tags:['KernelSU','Magisk','AnyKernel','SukiSU']},
  ...
  {icon:'🛠️',name:'Tooling & Env',num:'04',desc:'Dual-boot Ubuntu on ROG Strix G16 2025. Bash scripting, performance tuning, Git, terminal-first dev.',tags:['Bash','Ubuntu','Git','Linux']},
]
...
function Cursor(){
  ...
  useEffect(()=>{
    ...
    let raf
    const tick=()=>{
      const{mx,my,rx,ry}=s.current
      s.current.rx=rx+(mx-rx)*.12;s.current.ry=ry+(my-ry)*.12
      if(dr.current){dr.current.style.left=mx+'px';dr.current.style.top=my+'px'}
      if(rr.current){rr.current.style.left=s.current.rx+'px';rr.current.style.top=s.current.ry+'px'}
      raf=requestAnimationFrame(tick)
    }
    document.addEventListener('mousemove',mv);raf=requestAnimationFrame(tick)
    const bind=()=>document.querySelectorAll('a,button,.stc,.expi,.skli,.pjc,.soc').forEach(el=>{
...
function Loader({onDone}){
  ...
  useEffect(()=>{
    let v=0,id=setInterval(()=>{
      v+=Math.random()*14+7
      if(v>=100){v=100;clearInterval(id);setTimeout(()=>{setOut(true);setTimeout(onDone,600)},200)}
      setP(Math.round(v))
    },55)
...
function Sidebar(){
  ...
  useEffect(()=>{
    ...
    },{threshold:.25,rootMargin:'-5% 0px -55% 0px'})
...
function Hero(){
  return(
    <section id="home" className="hero">
      <div className="hbg"/>
      <div className="hportr">
        <div className="hportr-inner">
          <svg className="hpicon" width="90" height="90" viewBox="0 0 24 24" fill="none" stroke="rgba(232,40,40,0.4)" strokeWidth="0.6">
            <circle cx="12" cy="7" r="5"/><path d="M3 21c0-5 3.6-9 9-9s9 4 9 9"/>
          </svg>
        </div>
      </div>
      ...
      <div className="hcon">
        ...
        <h1 className="hnam">
          ...
          <span className="hl"><span className="hli">Jha.</span></span>
        </h1>
...
/* ── Stats — liquid glass, Image 3 style ─────────────────── */
function Stats(){
  const[v,setV]=useState([0,0,0]),ref=useRef(null),T=[200,6,3]
  useEffect(()=>{
    const ob=new IntersectionObserver(([e])=>{
      if(!e.isIntersecting)return
      T.forEach((t,i)=>{let n=0,step=Math.ceil(t/20),id=setInterval(()=>{n=Math.min(n+step,t);setV(prev=>{const nx=[...prev];nx[i]=n;return nx});if(n>=t)clearInterval(id)},40)})
      ob.disconnect()
    },{threshold:.4})
  ...
  const cards=[
    {label:'Community members',value:v[0],suffix:'+',note:'ReVork Telegram'},
    {label:'Active projects',value:v[1],suffix:'+',note:'Shipping right now'},
    {label:'Fan sites live',value:v[2],suffix:'',note:'sabrina · patidar · more'},
    {label:'Primary SoC',valueEl:<>SM<b>8350</b></>,note:'OnePlus 9 Pro target',small:true},
  ]
  return(
    <div id="stats" className="stats" ref={ref}>
      {cards.map((c,i)=>(
        <div className="stc" key={i}>
          ...
          <div>
            <div className={`stn${c.small?' stn-sm':''}`}>
              {c.valueEl||<>{c.value}<b>{c.suffix}</b></>}
            </div>
        ...
        {PROJ.map((p,i)=>(
          <div className={`pjc${p.rev?' rev':''}`} key={i}>
            <div className="pjv">
              ...
              <div className="pjvc">
                <div className="pjvt">{p.vt}</div>
                <div className="pjchips">{p.tags.map(t=><span key={t} className="chip">{t}</span>)}</div>
              </div>
            </div>
            <div className="pjb">
              <div className="pjtop">
              ...
              <div className="pjfoot">
                ...
                <a className="pjarrow" href={p.href} target="_blank" rel="noopener" aria-label={`View ${p.name}`}>
                  <ArwIco/>
                </a>
              </div>
            </div>
    ...
    <section id="skills">
      ...
      {SKILLS.map((s,i)=>(
        <div className="skli" key={i}>
          ...
          <div>
            <div className="sklnm">{s.name}</div>
            <p className="skldc">{s.desc}</p>
            <div className="sklch">{s.tags.map(t=><span key={t} className="chip">{t}</span>)}</div>
          </div>
...
function About(){
  return(
    <section id="about" style={{background:'rgba(255,255,255,.02)',borderTop:'1px solid var(--bord)'}}>
      ...
      <div className="abgr">
        <div className="abph">
          <svg width="70" height="70" viewBox="0 0 24 24" fill="none" stroke="rgba(232,40,40,0.22)" strokeWidth="0.7" style={{position:'relative',zIndex:1}}>
            <circle cx="12" cy="7" r="5"/><path d="M3 21c0-5 3.6-9 9-9s9 4 9 9"/>
          </svg>
          <div className="abphl">
            <div><div className="abphn">Aditya Jha</div><div className="abphs">Student · Developer</div></div>
            <div className="abphh">@ozyern</div>
          </div>
        </div>
        <div>
          <h3 className="abhl">Building <i>close to the metal</i> — kernels, ROMs, and fan sites that feel alive.</h3>
          <p className="abp">I'm Ozi an active Android ROM porter. I maintain <strong>ReVork</strong>, a bash-based framework for porting ColorOS/OxygenOS 16 to Snapdragon 888 devices, primarily the OnePlus 9 Pro.</p>
          ...
          <p className="abp">I run <strong>ReVork</strong>, a ~260-member Telegram community. Built <a href="https://sabrina.ozyern.me" target="_blank" rel="noopener">sabrina.ozyern.me</a> and cold-emailed her management. Iterating on <a href="https://patidar.ozyern.me" target="_blank" rel="noopener">patidar.ozyern.me</a>.</p>
          <div className="dvs">
            <div className="dvsl">Device lineup</div>
            <div className="dvsr"><span className="dvsrn">OnePlus 13</span><span className="dvsrk">"Espresso"</span></div>
            <div className="dvsr"><span className="dvsrn">OnePlus 9 Pro (lemonadep)</span><span className="dvsrk">"Feather"</span></div>
            <div className="dvsr"><span className="dvsrn">ROG Strix Scar 16 2025 · Ultra 9 275HX · RTX 5090</span><span className="dvsrk">Daily driver</span></div>
          </div>
        </div>
      </div>
    </section>
...
function Contact(){
  const lks=[
    ...
    {icon:<ML/>,name:'Email',sub:'ozyern.dev@gmail.com',href:'mailto:ozyern.dev@gmail.com'},
    {icon:<WB/>,name:'Website',sub:'ozyern.me',href:'https://ozyern.me'},
  ]
  const inf=[
    {l:'Status',v:<><span className="gdot"/>Available for collabs</>},
    {l:'Current Focus',v:'Feather Engine · RP21 Fan HQ'},
    {l:'Community',v:<a href="https://t.me/revork" target="_blank" rel="noopener">ReVork on Telegram ↗</a>},
    {l:'GitHub',v:<a href="https://github.com/ozyern" target="_blank" rel="noopener">github.com/ozyern ↗</a>},
    {l:'Fun fact',v:<span style={{color:'var(--muted)',fontWeight:400}}>Every project named after a Sabrina Carpenter song.</span>},
  ]
  return(
    <section id="contact" className="ctwrap">
      <div className="ctgr">
        <div>
          ...
          {lks.map(x=>(
            <a key={x.name} className="clk" href={x.href} target={x.href.startsWith('mailto')?undefined:'_blank'} rel="noopener">
              <div className="clkic">{x.icon}</div>
              <div className="clktx"><div className="clkn">{x.name}</div><div className="clks">{x.sub}</div></div>
              <div className="clka">↗</div>
            </a>
          ))}
        </div>
...
function Footer(){
  return(
    <footer>
      ...
      <div className="ftlk">
        {[['sabrina','https://sabrina.ozyern.me'],['patidar','https://patidar.ozyern.me'],['github','https://github.com/ozyern'],['top ↑','#home']].map(([l,h])=>(
